...
Use `$cdr` as the primary explicit invocation. `$code-rot-cleaner` remains a compatibility alias that delegates here. Skills are invoked with `$skill-name`; do not claim that literal `/cdr` is a supported custom slash command.
